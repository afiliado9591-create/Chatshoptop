/* ChatShop — produto único leve: imagem principal + 4 miniaturas + cartão e 3 ações. Não altera collect/publicação. */
(function(){
'use strict';
...
body.chatshop-single-reference #pubFeed{height:100dvh!important;scroll-snap-type:y mandatory!important;overscroll-behavior-y:contain!important}
body.chatshop-single-reference #pubFeed .pub-slide{height:100dvh!important;min-height:100dvh!important;scroll-snap-align:start!important;scroll-snap-stop:always!important;background:#fff!important;display:block!important;position:relative!important;overflow:hidden!important;padding:0!important;content-visibility:auto;contain-intrinsic-size:100dvh}
.spr-page{position:absolute;inset:0;display:flex;flex-direction:column;background:#fff;z-index:20;padding:10px 10px 14px;box-sizing:border-box;gap:8px;opacity:0;pointer-events:none;transition:opacity .1s ease}
...
.spr-buy{background:var(--store-buy,var(--store-main,#7A2E3B));color:#fff}.spr-listen{background:#2563eb;color:#fff;border:1px solid #1d4ed8!important}.spr-listen.speaking{background:#1d4ed8;transform:scale(.98)}
.spr-chat{background:#e11d48;color:#fff;border:1px solid #be123c!important;box-shadow:0 4px 14px rgba(225,29,72,.30)!important;transition:background .18s ease,transform .18s ease,box-shadow .18s ease}.spr-chat:active{transform:scale(.98)}.spr-chat.chat-open{background:#7c3aed!important;border-color:#6d28d9!important;box-shadow:0 5px 18px rgba(124,58,237,.38)!important;transform:translateY(-1px)}
...
let currentIndex=0;
function render(){if(!isAffiliateSingle())return false;ensureStyle();document.body?.classList.remove('chatshop-single-gallery');document.body?.classList.add('chatshop-single-reference');forceHideGlobalChat();const ps=products(),slides=$$('#pubFeed .pub-slide');slides.forEach((slide,index)=>buildSlide(slide,ps[index]||{},index));if(ps[1])preloadMain(ps[1]);syncChatButtonState();const feed=$('#pubFeed');if(feed&&!feed.dataset.sprScrollBound){feed.dataset.sprScrollBound='1';let raf=0;feed.addEventListener('scroll',()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{const h=Math.max(1,feed.clientHeight);const idx=Math.max(0,Math.min(ps.length-1,Math.round(feed.scrollTop/h)));if(idx!==currentIndex){currentIndex=idx;if(ps[idx+1])preloadMain(ps[idx+1])}})},{passive:true})}return true}
function boot(){let tries=0;const start=()=>{tries++;if(render()||tries>=40){clearInterval(timer);return}};const timer=setInterval(start,150);start();const feed=$('#pubFeed');if(feed&&!feed.dataset.sprObserverBound){feed.dataset.sprObserverBound='1';new MutationObserver(()=>requestAnimationFrame(render)).observe(feed,{childList:true})}window.addEventListener('load',render,{once:true});document.addEventListener('click',()=>setTimeout(syncChatButtonState,20),true)}
