...
function imagesOf(p){
  ...
  if(Array.isArray(p?.additionalImages))list.push(...p.additionalImages);
  list.push(p?.image,p?.imageUrl,p?.imagem,p?.image2,p?.image3,p?.image4);
  return [...new Set(list.map(normUrl).filter(Boolean))].slice(0,4);
}
...
function buildSlide(slide,p,index){
  const imgs=imagesOf(p);if(!imgs.length)return;
  let page=slide.querySelector('.spr-page');
  const key=JSON.stringify([imgs,p?.name,p?.price,p?.link,p?.buttonText,p?.cardColor,p?.cardOpacity]);
  if(page?.dataset.key===key)return;
  page?.remove();page=document.createElement('div');page.className='spr-page';page.dataset.key=key;
  ...
  page.innerHTML=`<div class="spr-main"><img src="${esc(mainSrc)}" alt="${esc(p?.name||'Produto')}"></div><div class="spr-thumbs"></div><div class="spr-card" style="background:${cardBg};color:${esc(cardText)}"><div class="spr-name">${esc(p?.name||'Produto')}</div><div class="spr-price">${esc(p?.price||'Consulte')}</div><div class="spr-actions"><button type="button" class="spr-listen">🔊 Ouvir descrição</button><button type="button" class="spr-chat">💬 Falar</button><a class="spr-buy" target="_blank" rel="noopener">🛒 Comprar</a></div></div>`;
  const mainImg=$('.spr-main img',page),thumbs=$('.spr-thumbs',page);
  imgs.slice(1,4).forEach((src,i)=>{
    const b=document.createElement('button');b.type='button';b.className='spr-thumb';
    b.innerHTML=`<img src="${esc(src)}" alt="${esc(p?.name||'Produto')} ${i+2}">`;
    b.onclick=()=>{mainImg.src=src;$$('.spr-thumb',page).forEach(x=>x.classList.remove('active'));b.classList.add('active')};
  ...
  $('.spr-chat',page).onclick=()=>openChat(p,index,slide);
  const buy=$('.spr-buy',page),original=slide.querySelector('.pub-slide-buy');
  buy.href=String(p?.link||original?.href||'#');buy.textContent='🛒 '+String(p?.buttonText||'Comprar');
  buy.onclick=e=>{if(original&&(!p?.link||p.link==='#')){e.preventDefault();original.click()}};
