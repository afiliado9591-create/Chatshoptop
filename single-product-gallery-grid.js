...
.spr-chat-product-card{display:grid;grid-template-columns:70px 1fr;gap:10px;align-items:center;background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:9px;box-shadow:0 2px 8px rgba(0,0,0,.08);max-width:100%}
.spr-chat-product-card img{width:70px;height:70px;object-fit:contain;border-radius:10px;background:#f8fafc}
.spr-chat-product-info{min-width:0}.spr-chat-product-name{font-size:13px;font-weight:900;line-height:1.2;color:#111827}.spr-chat-product-price{font-size:14px;font-weight:950;color:var(--store-main,#c2185b);margin:4px 0 7px}.spr-chat-product-buy{display:inline-flex;align-items:center;justify-content:center;min-height:36px;padding:8px 12px;border-radius:10px;background:var(--store-buy,var(--store-main,#7A2E3B));color:#fff!important;text-decoration:none;font-size:12px;font-weight:900}
...
function injectChatProductCard(p,index,slide){const messages=$('#spfProductChatMessages');if(!messages)return false;messages.querySelector('.spr-chat-product-card')?.remove();const card=document.createElement('div');card.className='spr-chat-product-card';const src=imagesOf(p)[0]||'';card.innerHTML=`${src?`<img src="${esc(src)}" alt="${esc(p?.name||'Produto')}" loading="lazy">`:''}<div class="spr-chat-product-info"><div class="spr-chat-product-name">${esc(p?.name||'Produto')}</div><div class="spr-chat-product-price">${esc(p?.price||'Consulte')}</div><a class="spr-chat-product-buy" href="#">🛒 Comprar</a></div>`;const buy=$('.spr-chat-product-buy',card),original=slide?.querySelector('.pub-slide-buy');const href=String(p?.link||original?.href||'#');buy.href=href;buy.target=href&&href!=='#'?'_blank':'';buy.rel='noopener';buy.onclick=e=>{if(original&&(!p?.link||p.link==='#')){e.preventDefault();original.click()}};messages.prepend(card);messages.scrollTop=0;return true}
function openChat(p,index,slide){window.__CHATSHOP_ACTIVE_PRODUCT=p;window.__CHATSHOP_ACTIVE_PRODUCT_INDEX=index;const chat=slide?.querySelector('.spf-aff-chat');if(chat){chat.click();setTimeout(()=>injectChatProductCard(p,index,slide),60);return}const overlay=$('#spfProductChat');if(overlay){const title=$('#spfProductChatTitle');if(title)title.textContent='💬 '+String(p?.name||'Produto');overlay.classList.add('open');setTimeout(()=>injectChatProductCard(p,index,slide),30);return}const hiddenGlobal=$('#pubChatToggle');if(hiddenGlobal){hiddenGlobal.dataset.productIndex=String(index);hiddenGlobal.dataset.productName=String(p?.name||'');hiddenGlobal.click();forceHideGlobalChat();setTimeout(()=>injectChatProductCard(p,index,slide),80)}}
function buildSlide(slide,p,index){const imgs=imagesOf(p);if(!imgs.length)return;let page=slide.querySelector('.spr-page');const key=JSON.stringify([imgs,p?.name,p?.price,p?.link,p?.buttonText,p?.cardColor,p?.cardOpacity,p?.showSellerButton]);if(page?.dataset.key===key)return;slide.classList.remove('spr-ready');page?.remove();page=document.createElement('div');page.className='spr-page';page.dataset.key=key;const d=data()||{};const opacity=Math.max(0,Math.min(1,Number(d.cardOpacity==null?95:d.cardOpacity)/100));const cardBg=hexToRgba(d.cardColor||'#FFFFFF',opacity);const cardText=d.cardTextColor||'#1A1A1A';const mainSrc=imgs[0],mainLoading=index===0?'eager':'lazy',priority=index===0?'high':'auto';page.innerHTML=`<div class="spr-main"><img src="${esc(mainSrc)}" alt="${esc(p?.name||'Produto')}" loading="${mainLoading}" fetchpriority="${priority}" decoding="async"></div><div class="spr-thumbs"></div><div class="spr-card" style="background:${cardBg};color:${esc(cardText)}"><div class="spr-name">${esc(p?.name||'Produto')}</div><div class="spr-price">${esc(p?.price||'Consulte')}</div><div class="spr-actions"><button type="button" class="spr-listen">🔊 Ouvir descrição</button><button type="button" class="spr-chat">💬 Falar com vendedor</button><a class="spr-buy" target="_blank" rel="noopener">🛒 Comprar</a></div></div>`;const mainImg=$('.spr-main img',page),thumbs=$('.spr-thumbs',page);
imgs.slice(0,4).forEach((src,i)=>{const b=document.createElement('button');b.type='button';b.className='spr-thumb'+(i===0?' active':'');b.innerHTML=`<img src="${esc(src)}" alt="${esc(p?.name||'Produto')} ${i+1}" loading="lazy" decoding="async">`;b.onclick=async()=>{if(mainImg.src===new URL(src,location.href).href){$$('.spr-thumb',page).forEach(x=>x.classList.remove('active'));b.classList.add('active');return}const ok=await preload(src);if(ok){mainImg.src=src;$$('.spr-thumb',page).forEach(x=>x.classList.remove('active'));b.classList.add('active')}};thumbs.appendChild(b)});
...
function currentSlideIndex(slides){const center=innerHeight/2;let best=0,dist=Infinity;slides.forEach((s,i)=>{const r=s.getBoundingClientRect(),d=Math.abs((r.top+r.bottom)/2-center);if(d<dist){dist=d;best=i}});return best}
function installLightPreload(ps,slides){const feed=$('#pubFeed');if(!feed||feed.dataset.sprLightPreload)return;feed.dataset.sprLightPreload='1';let raf=0,last=-1;const warm=()=>{const i=currentSlideIndex(slides);if(i===last)return;last=i;const next=imagesOf(ps[i+1]||{})[0];if(next)preload(next)};feed.addEventListener('scroll',()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(warm)},{passive:true});warm()}
function decorate(){if(!isAffiliateSingle())return false;ensureStyle();document.body?.classList.remove('chatshop-single-gallery');document.body?.classList.add('chatshop-single-reference');forceHideGlobalChat();const ps=products(),slides=$$('#pubFeed .pub-slide');slides.forEach((slide,index)=>buildSlide(slide,ps[index]||{},index));installLightPreload(ps,slides);return true}
function boot(){let n=0;const t=setInterval(()=>{n++;decorate();forceHideGlobalChat();if(n>80)clearInterval(t)},180);if(document.body)new MutationObserver(()=>requestAnimationFrame(()=>{decorate();forceHideGlobalChat()})).observe(document.body,{childList:true,subtree:true});window.addEventListener('load',()=>{decorate();forceHideGlobalChat()},{once:true})}
