...
function preloadAround(ps,index){[index,index+1,index+2,index-1].forEach(i=>{if(ps[i])preloadProduct(ps[i])})}
function decorate(){if(!isAffiliateSingle())return false;ensureStyle();document.body?.classList.remove('chatshop-single-gallery');document.body?.classList.add('chatshop-single-reference');forceHideGlobalChat();const ps=products();$$('#pubFeed .pub-slide').forEach((slide,index)=>{preloadAround(ps,index);buildSlide(slide,ps[index]||{},index)});return true}
