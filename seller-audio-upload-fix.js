...
function loadPublishDiagnostic(){
  if(document.querySelector('script[data-chatshop-publish-diagnostic]'))return;
  const script=document.createElement('script');
  script.src='/publish-diagnostic.js?v=20260813-1';
  script.defer=true;
  script.dataset.chatshopPublishDiagnostic='1';
...
function readableStorageError(error){
  ...
  if(code==='storage/unauthorized')return'O Firebase Storage recusou o envio por permissão.';
  if(code==='storage/unauthenticated')return'Sua sessão não foi reconhecida pelo Storage. Entre novamente e tente de novo.';
  if(code==='storage/quota-exceeded')return'O Firebase Storage está indisponível por plano/cota.';
  ...
  if(code==='storage/retry-limit-exceeded'||code==='storage/canceled')return'O Firebase Storage demorou demais para responder. O envio foi interrompido.';
...
async function uploadWithTimeout(blob,fileName,section){
  const uid=currentUid(),status=statusEl(section),hidden=hiddenUrl(section);
  ...
  try{
    ...
    const snap=await new Promise((resolve,reject)=>{
      ...
      },error=>{if(finished)return;finished=true;clearTimeout(timer);reject(error)},()=>{if(finished)return;finished=true;clearTimeout(timer);resolve(task.snapshot)});
    });
    ...
    setPreview(section,url,'✅ Áudio salvo','Ouça para conferir. Agora clique em Salvar/Publicar a loja.');
    ...
    const box=previewBox(section);
    if(box){box.style.display='block';const note=box.querySelector('.seller-audio-preview-note');if(note)note.textContent='A gravação ficou disponível apenas para você ouvir agora, mas ainda NÃO foi salva na loja.'}
    notify(msg);return'';
  }
}
async function startRecording(button,section){
  ...
  try{
    ...
    rec.onstop=async()=>{
      clearTimeout(activeRecord?.timer);stream.getTracks().forEach(t=>t.stop());activeRecord=null;
    ...
    rec.start(500);button.textContent='⏹️ Parar gravação';button.classList.remove('success');
...
function bind(section){
  ...
  if(record)record.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();event.stopPropagation();startRecording(record,section)},true);
  ...
  if(file)file.addEventListener('change',event=>{
    ...
    setLocalPreview(section,f);uploadWithTimeout(f,f.name,section);
  },true);
}
function scan(){document.querySelectorAll('.seller-audio-editor').forEach(bind)}
loadPublishDiagnostic();
