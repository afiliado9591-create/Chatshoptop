...
`;
  ...
  patched = patched.replace(oldFallback, newFallback);
  return patched;
}
...
module.exports = async function handler(request, response) {
  try {
    ...
    if (file !== 'index' && file !== 'virtual') return response.status(400).send('Arquivo inválido');
    const filename = file === 'index' ? 'index.html' : 'virtual-chat-description-fix.js';
    ...
    console.error('Q&A source patch error:', error);
    return response.status(500).send('Falha ao preparar o mecanismo de Q&A.');
  }
};
