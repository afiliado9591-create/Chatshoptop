...
// Verifica só o token (usado no GET)
...
// Verifica token + exige admin (usado no POST) — antes ausente, causava ReferenceError
async function authAdmin(req) {
  const decoded = await authUser(req);
  const db = getAdmin().firestore();
  const userSnap = await db.collection('users').doc(decoded.uid).get();
  const userDoc = userSnap.exists ? userSnap.data() || {} : {};
  if (!isAdmin(decoded, userDoc)) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    // ---- POST: admin adiciona produto no catálogo ----
    if (req.method === 'POST') {
      ...
      if (!catalogoId || !name || !image) {
        return res.status(400).json({ error: 'missing_fields', message: 'Nome, imagem e catálogo são obrigatórios.' });
      }
      ...
      const keywords = Array.isArray(b.keywords)
        ? b.keywords.map(x => clean(x, 80)).filter(Boolean).slice(0, 30)
        : [];
    ...
    // ---- GET: listar catálogos ou produtos de um catálogo ----
    ...
    return res.status(status).json({ error: 'catalog_failed', message: String(e.message || 'Não foi possível carregar o catálogo.') });
  }
};
