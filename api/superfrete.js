...
module.exports = async function handler(req, res) {
  ...
  try {
    const action = String(req.query?.action || 'quote').trim();
    ...
    console.error('Erro na integração SuperFrete:', error);
    const message = String(error?.message || 'Não foi possível processar a solicitação agora.');
