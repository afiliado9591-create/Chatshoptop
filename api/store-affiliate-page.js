...
async function getByCustomDomain(h){
  ...
  for(const candidate of[...new Set(candidates)]){
    ...
    if(doc){
      const data=docData(doc),slug=String(data.slug||String(doc.name||'').split('/').pop()||'').trim();
      if(slug)return{slug,data};
    }
  }
  return null;
}
...
module.exports=async function(req,res){
  try{
    ...
    if(!found)return res.status(404).send('Loja não encontrada.');
...
function code(){return'af_'+Date.now().toString(36)+Math.random().toString(36).slice(2,7)}
...
sendEl.addEventListener('click',async()=>{
  ...
  const w=String(whatsappEl.value||'').replace(/\\D/g,'');
  const e=String(emailEl.value||'').trim();
  const so=String(socialEl.value||'').trim();
  errEl.textContent='';
  ...
  try{
    ...
    const c=code();
    const entry={type:'affiliate_application',affiliateCode:c,affiliateName:n,affiliateWhatsapp:w,affiliateEmail:e,affiliateSocial:so,affiliateStatus:'active',data:firebase.firestore.FieldValue.serverTimestamp()};
    await db.collection('chatshops').doc(SLUG).collection('leads').add(entry);
    try{
      const directory={};
      directory[c]={affiliateCode:c,affiliateName:n,affiliateWhatsapp:w,affiliateEmail:e,affiliateSocial:so,affiliateStatus:'active'};
      await db.collection('chatshops').doc(SLUG).set({affiliateDirectory:directory},{merge:true});
    }catch(directoryError){console.warn('Cadastro salvo; diretório auxiliar não pôde ser atualizado.',directoryError)}
    const general=location.origin+'/?ref='+encodeURIComponent(c);
    formEl.style.display='none';okEl.style.display='block';linkEl.textContent=general;
    copyEl.onclick=()=>copyText(general,copyEl);
    ...
    console.error('Cadastro de afiliado:',x);
    errEl.textContent='Não foi possível concluir o cadastro agora. Tente novamente em alguns instantes.';
    sendEl.disabled=false;sendEl.textContent=${JSON.stringify(cta)};
  }
});
<\/script>`;
