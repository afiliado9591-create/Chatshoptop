/* Proxy do ChatShop: mantém o render atual e injeta os recursos da Loja Virtual. */
module.exports = async function handler(request, response) {
  try {
    ...
    const upstream = await fetch(target, {
      headers: {
        accept: 'text/html',
        'x-chatshop-proxy': 'virtual-shipping'
      }
    });
    let html = await upstream.text();
    const tag = '<script src="/virtual-shipping-upgrade.js?v=20260813-1707"></script>';
    if (!html.includes('/virtual-shipping-upgrade.js')) {
      const pos = html.toLowerCase().lastIndexOf('</body>');
      html = pos >= 0 ? html.slice(0, pos) + tag + '\n' + html.slice(pos) : html + '\n' + tag;
    }
    ...
    console.error('Erro no proxy ChatShop:', error);
