...
async function requestWithdrawal(amount){const d=dbRef();if(!d||!uid())throw new Error('Entre na sua conta.');if(!(await hasProfile()))throw new Error('Crie seu perfil de divulgador primeiro.');const active=(await getWithdrawals()).find(x=>x.status==='pendente');if(active)throw new Error('Você já possui um saque pendente.');await d.collection('shopadsWithdrawals').add({promoterUid:uid(),promoterEmail:email(),amount:Number(amount||0),status:'pendente',mode:'teste',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()})}
function styles(){if($('#shopAdsWalletStyles'))return;const s=document.createElement('style');s.id='shopAdsWalletStyles';s.textContent=`#shopAdsWallet{position:fixed;inset:0;z-index:10080;background:#f7f5ff;overflow:auto;display:none;font-family:Arial;color:#1f2937}#shopAdsWallet.open{display:block}.saw-top{background:linear-gradient(135deg,#166534,#16a34a);color:#fff;padding:18px}.saw-head,.saw-body{max-width:760px;margin:auto}.saw-head{display:flex;justify-content:space-between;align-items:center}.saw-close{border:0;background:#ffffff25;color:#fff;width:40px;height:40px;border-radius:50%;font-size:20px}.saw-body{padding:16px}.saw-card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:16px;margin-bottom:12px}.saw-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.saw-metric{padding:14px;border-radius:12px;background:#f9fafb;text-align:center;font-size:12px}.saw-metric b{display:block;font-size:22px}.saw-row{padding:12px 0;border-top:1px solid #eee;font-size:13px}.saw-btn{border:0;border-radius:10px;padding:12px;font-weight:900;background:#16a34a;color:#fff;width:100%}.saw-note{font-size:12px;color:#6b7280}@media(max-width:560px){.saw-grid{grid-template-columns:1fr}}`;document.head.appendChild(s)}
...
function walletButtons(){return Array.from(document.querySelectorAll('button,a')).filter(el=>!el.closest('#shopAdsWallet,#promoterDirectoryModal,#shopAdsOverlay')&&String(el.textContent||'').replace(/\s+/g,' ').trim().toLowerCase().includes('carteira shopads'))}
function dedupeWalletButtons(){const list=walletButtons();if(!list.length)return null;let keep=list.find(x=>x.id==='shopAdsWalletBtn')||list[0];keep.id='shopAdsWalletBtn';keep.onclick=openWallet;list.forEach(x=>{if(x!==keep)x.remove()});return keep}
async function syncButton(){let existing=dedupeWalletButtons();const shop=$('#shopAdsBtn,[data-open-shopads]');if(!(await hasProfile())){walletButtons().forEach(x=>x.remove());return false}existing=dedupeWalletButtons();if(existing)return true;if(!shop||!shop.parentElement)return false;const b=document.createElement('button');b.id='shopAdsWalletBtn';b.className=shop.className||'btn';b.textContent='💰 Carteira ShopAds';b.onclick=openWallet;shop.parentElement.insertBefore(b,shop.nextSibling);dedupeWalletButtons();return true}
function install(){window.openShopAdsWallet=openWallet;syncButton();window.addEventListener('chatshop:promoter-profile-saved',syncButton);new MutationObserver(()=>dedupeWalletButtons()).observe(document.documentElement,{childList:true,subtree:true});let n=0,t=setInterval(()=>{syncButton();if(++n>40)clearInterval(t)},300)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install);else install();
})();
